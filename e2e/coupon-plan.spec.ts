...
async function mockReads(page: Page) {
  await page.route(/\/api\/couponPlan\/p1\/status(\?|$)/, sendData(detail));
  await page.route(/\/api\/couponPlan\/p1\/param(\?|$)/, sendData(param));
  await page.route(/\/api\/couponPlan\/p1(\?|$)/, sendData(detail));
  await page.route(/\/api\/couponSheet\/s1\/basicInfo(\?|$)/, (route) =>
...
const BASIC_URL = "/zh-HK/plans/coupon/basicInfo?planId=p1&sheetId=s1";
const PARAM_URL = "/zh-HK/plans/coupon/param?planId=p1&sheetId=s1";

test.describe("/plans/coupon (dividend)", () => {
  ...
  test("basic-info shows the expired card for an unpaid plan", async ({
    ...
    void paymentDetail;
    await page.route(/\/api\/couponPlan\/p1\/status(\?|$)/, sendData(expired));
    await page.goto(BASIC_URL);
  ...
  test("submitting basic info navigates to the param step", async ({
    ...
    await expect(page).toHaveURL(
      /\/plans\/coupon\/param\?planId=p1&sheetId=s1/,
    );
  });
  ...
  test("the expected-installment change PUTs /cal with a bare-number application/json body", async ({
    ...
    await page.route(/\/api\/couponSheet\/s1\/cal(\?|$)/, (route) => {
  ...
  test("a premium mismatch reveals adjust, which PUTs /calAdjust", async ({
    ...
    await page.route(
      /\/api\/couponSheet\/s1\/cal(\?|$)/,
      sendData({ instal: "4999", instal_num: 4999, amount: "100000" }),
    );
    await page.route(/\/api\/couponSheet\/s1\/calAdjust(\?|$)/, (route) => {
      adjustCalled = true;
      return sendData({ instal: "5000", instal_num: 5000, amount: "100000" })(
        route,
      );
  ...
  test("generate-sheet navigates to the coupon sheet route", async ({
    page,
  }) => {
    await page.route(
      /\/api\/couponSheet\/s1\/info(\?|$)/,
    ...
    await expect(page).toHaveURL(
      /\/plans\/coupon\/sheet\?planId=p1&sheetId=s1/,
    );
  });
});
