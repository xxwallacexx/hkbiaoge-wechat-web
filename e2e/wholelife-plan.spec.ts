...
async function mockReads(page: Page) {
  await page.route(/\/api\/wholelifePlan\/p1\/status(\?|$)/, sendData(detail));
  await page.route(/\/api\/wholelifePlan\/p1\/param(\?|$)/, sendData(param));
  await page.route(/\/api\/wholelifePlan\/p1(\?|$)/, sendData(detail));
  await page.route(/\/api\/wholelifeSheet\/s1\/basicInfo(\?|$)/, (route) =>
...
const BASIC_URL = "/zh-HK/plans/wholelife/basicInfo?planId=p1&sheetId=s1";
const PARAM_URL = "/zh-HK/plans/wholelife/param?planId=p1&sheetId=s1";

test.describe("/plans/wholelife (whole life)", () => {
  ...
  test("submitting basic info navigates to the param step", async ({
    ...
    await expect(page).toHaveURL(
      /\/plans\/wholelife\/param\?planId=p1&sheetId=s1/,
    );
  });
  ...
  test("the expected-installment change PUTs /cal with a bare-number application/json body", async ({
    ...
    await page.route(/\/api\/wholelifeSheet\/s1\/cal(\?|$)/, (route) => {
  ...
  test("generate-sheet navigates to the whole-life sheet route", async ({
    page,
  }) => {
    await page.route(
      /\/api\/wholelifeSheet\/s1\/info(\?|$)/,
    ...
    await expect(page).toHaveURL(
      /\/plans\/wholelife\/sheet\?planId=p1&sheetId=s1/,
    );
  });
});
