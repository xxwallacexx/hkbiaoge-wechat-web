...
import { updateSavingPlanSheetWithdrawal } from "@/lib/api/saving-plans";

/**
 * Sets a withdrawal across one or more consecutive years for a row in the worksheet. On
 * success it closes the dialog (via the trigger ref) and invalidates the sheet so the table
 * re-derives — replacing the webview `revalidatePage` server action.
 */
export function WithdrawalForm({
  ...
  const { mutate: onSubmit, isPending: isLoading } = useMutation({
    mutationFn: (vars: { startRow: number; endRow: number; value: number }) =>
      updateSavingPlanSheetWithdrawal({ sheetId, ...vars }),
    onSuccess: () => {
