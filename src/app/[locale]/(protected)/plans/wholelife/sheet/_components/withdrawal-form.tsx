...
import { updateWholelifePlanSheetWithdrawal } from "@/lib/api/wholelife-plans";
...
export function WithdrawalForm({
  ...
  const { mutate: onSubmit, isPending: isLoading } = useMutation({
    mutationFn: (vars: { startRow: number; endRow: number; value: number }) =>
      updateWholelifePlanSheetWithdrawal({ sheetId, ...vars }),
    onSuccess: () => {
      closeRef.current?.click();
      queryClient.invalidateQueries({
        queryKey: ["wholelifePlanSheet", sheetId],
      });
    },
