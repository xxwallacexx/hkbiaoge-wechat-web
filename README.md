...
- **PDF hand-off:** tapping a PDF (產品單頁 / 優惠推廣) rewrites its OSS url onto
  `oss.hkbiaoge.com` — a custom domain aliased onto the bucket, and unlike the shared
  `*.aliyuncs.com` host one we can verify — then navigates the web-view straight to the
  document, and posts `{ type: "openPdf", url, name }` over the bridge. The client only
  has to add **`oss.hkbiaoge.com` to 业务域名**; no native page is needed. Full spec:
  [`docs/mini-program-pdf-handoff.md`](docs/mini-program-pdf-handoff.md); the host map
  lives in one constant (`OSS_HOST_ALIASES`, `src/lib/oss.ts`).
...
- [ ] Domain mapped to the Cloud Run service, then added to the Mini Program
      **业务域名 whitelist**
- [ ] `oss.hkbiaoge.com` 备案-filed, its WeChat verification `.txt` uploaded to the
      bucket root, and the domain added to the **业务域名 whitelist** — until then,
      tapping a brochure or promotion opens a blank page inside WeChat
      (see [`docs/mini-program-pdf-handoff.md`](docs/mini-program-pdf-handoff.md))
